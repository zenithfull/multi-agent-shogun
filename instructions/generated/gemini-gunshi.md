...
This section describes Gemini-specific tools and features available in the custom CLI wrapper.

## Tool Usage

Gemini CLI provides specialized functions for file operations and system interaction. You should use these tools whenever possible instead of suggesting manual commands to the user, although you can also run shell commands directly.

- **read_file(filepath)**: Read the content of a file.
- **write_file(filepath, content)**: Create or overwrite a file with the specified content.
- **list_directory(path)**: List files and directories in the specified path.
- **execute_shell_command(command)**: Execute a shell command and get the output (stdout/stderr).
- **grep_search(pattern, path)**: Search for a string pattern in files within a directory (recursive).

## Tool Guidelines

1.  **Read before Write**: Always read a file before modifying it to ensure you don't lose existing context.
2.  **Use `write_file` for creating/editing**: This tool handles file creation and overwriting. For partial edits, read the file first, modify the content in memory, and then write the full content back.
3.  **Use `execute_shell_command` for system tasks**: Use this for git operations, running tests, or other system commands.
4.  **Use `grep_search` for code exploration**: Efficiently find relevant code snippets.

## Important Notes

- **Context Preservation**: The CLI wrapper maintains conversation history, but for long tasks, it's good practice to summarize progress in the `dashboard.md` or task reports.
- **File Paths**: Always use relative paths from the project root or absolute paths.
- **Environment**: You are running in a Python environment with access to standard libraries.

## Interaction Protocol

- **Self-Correction**: If a tool call fails, analyze the error message and try again with corrected parameters.
- **Verification**: After writing a file or running a command, verify the result (e.g., read the file back or check command exit code/output).
